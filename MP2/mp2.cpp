...
void init(void)
{
	...
	gluLookAt (-0.5, 0.0, 0.25, 1.0, 0.0, 0.25, 0.0, 0.0, 1.0);
...
void display(void)
{
	...
	//
	// Apply rotation and translation matrices to fly the plane
	GLfloat mv[16];  //create a matrix for the MV to be stored in
	glGetFloatv(GL_MODELVIEW_MATRIX, mv); //grab the MV matrix
	glLoadIdentity(); // load identity
	glTranslatef(0.0,0.00,0.0075);  // Move the plane forward by translating
	if(roll)
		glRotatef(roll_angle, 0.0, 0.0, roll); // Roll the plane by rotating about z axis
	if(pitch)
		glRotatef(pitch_angle, pitch, 0.0, 0.0); // Pitch the plane by rotating about x axis
	glMultMatrixf(mv);
	//
...
/**
	Waits for a key to be released and then sets the approprate roll/pitch values back to zero
	Note if both up/down or left/right are held down at same time, releasing one will stop the pitch/roll
**/
...
/**
	Set the roll and pitch based on which key is pressed.
	Note that holding right and left at same time will rotate in direction of last key pressed
